...
export function BookingFiltersBar({ filters, onChange, viewMode, onViewModeChange }: Props) {
      ...
      <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
        ...
        <div className="flex items-center px-3 py-2 bg-[#F5F7FA] rounded-lg border border-transparent focus-within:border-[#1E3A5F]/30 transition-colors">
          ...
          <input
            ...
            className="bg-transparent border-none outline-none w-32 text-sm text-text-main placeholder-gray-400"
          />
        </div>
      ...
      <div className="flex bg-[#F5F7FA] rounded-lg p-1 shrink-0 ml-auto lg:ml-0">
        <button
          ...
          className={`p-1.5 rounded-md transition-colors flex items-center ${
            viewMode === "list" ? "bg-white shadow-sm text-primary" : "text-text-muted hover:text-text-main"
          }`}
          title="Vista Lista"
        >
          <List size={16} />
        ...
        <button
          ...
          className={`p-1.5 rounded-md transition-colors flex items-center ${
            viewMode === "calendar" ? "bg-white shadow-sm text-primary" : "text-text-muted hover:text-text-main"
          }`}
