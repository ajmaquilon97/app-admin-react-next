...
export function CrearEspacioWizard({
  ...
  // — Paso 2: Ubicación —
  const [provinciaId, setProvinciaId] = useState<number | null>(provincias[0]?.id ?? null);
  const [ciudadId, setCiudadId] = useState<number | null>(
    provincias[0]?.ciudades[0]?.id ?? null
  );
  ...
  const canAdvanceStep2 = !!(provinciaId && ciudadId && referencia.trim().length >= 5 && coords);
              ...
              {step === 3 && (
                <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-muted mt-1.5">
                      {!canSubmitStep3
                        ? "Ingresa un número entero de al menos 1 persona."
                        : esCupoCompartido
