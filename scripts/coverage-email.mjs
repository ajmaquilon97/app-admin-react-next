 ...
 *
 * Uso:
 *   npx jest --coverage --ci --runInBand --json --outputFile=jest-results.json
 *   node scripts/coverage-email.mjs jest-results.json
 *
 * Produce dos artefactos en `coverage/`:
 ...
 *     la ejecución de GitHub Actions ($GITHUB_STEP_SUMMARY).
 *
...
const md = [];
md.push(`## ${exito ? "✅" : "❌"} Pruebas y cobertura — portal web`);
md.push("");
md.push(
...
md.push("");
md.push("| Módulo | Tests | Pasados | Fallidos | Sentencias (%) | Ramas (%) | Funciones (%) | Líneas (%) |");
md.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
...
md.push(
  `| **TOTAL PROYECTO WEB** | **${totalTests.total}** | **${totalTests.pasados}** | ` +
    `**${totalTests.fallidos}** | ` +
    METRICAS.map((m) => `**${num(totalMetricas[m])}**`).join(" | ") +
    " |",
);
md.push(
  `| Umbral mínimo requerido | — | — | — | ${UMBRAL} | ${UMBRAL} | ${UMBRAL} | ${UMBRAL} |`,
);
md.push("");
...
const metricaCelda = (valor) =>
  celda(num(valor), `text-align:right;color:${valor >= UMBRAL ? VERDE : ROJO};font-variant-numeric:tabular-nums`);
...
const filaTotal =
  ...
  celda("TOTAL PROYECTO WEB", "font-weight:700") +
...
const filaUmbral =
  `<tr>` +
  celda(`Umbral mínimo requerido (tesis §8.6.1)`, `color:${GRIS}`) +
...
<body style="margin:0;padding:24px;background:#f8fafc;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1f2937">
  <div style="max-width:860px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
    ...
    <div style="padding:18px 22px;background:${exito ? "#ecfdf5" : "#fef2f2"};border-bottom:1px solid #e2e8f0">
      <div style="font-size:12px;text-transform:uppercase;letter-spacing:.06em;color:${GRIS}">Reporte de pruebas — Portal Web (Next.js)</div>
...
const asunto =
  `[${exito ? "OK" : "FALLO"}] Cobertura portal web · ${CONTEXTO.rama} · ` +
